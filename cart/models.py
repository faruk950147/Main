...
class Cart(models.Model):
    ...
    def total_price(self):
        ...
        total = sum(
            ...
            for item in self.items.all() if item.product  # Ensure product is not None
        )

        # Apply coupon discount if valid
        if self.coupon and self.coupon.is_valid(total):
            total -= min(self.coupon.coupon_discount, total)  # Discount can't exceed total
...
class CartItem(models.Model):
    cart = models.ForeignKey(Cart, related_name="items", on_delete=models.CASCADE)  # FIX: SET_NULL → CASCADE
    ...
    def total_price(self):
        ...
        return self.quantity * (self.variant.price if self.variant and self.variant.price else self.product.price)
